...
export const metadata: Metadata = {
  title: 'Frequently Asked Questions | Notary Public Islington',
...
const faqs = [
  {
    ...
    answer: `The cost of notarising a document in the UK varies depending on the complexity of the work and time required. At Notary Public Islington, the minimum fee is £80 (no VAT) for up to twenty minutes of notary time. Pro-rata rates apply for longer appointments.
...
For an accurate quote, please contact us with details of your specific requirements. We provide transparent pricing and will always confirm fees before proceeding with any work.`,
  },
  {
    ...
    question: 'What services does a notary public offer in Islington?',
    answer: `A notary public in Islington offers a wide range of document authentication and certification services for both businesses and individuals. These include:
...
Contact us with details of your requirements for a more accurate time estimate.`,
  },
  {
    ...
    answer: `Yes, I offer home and business visits throughout Islington and surrounding London boroughs for an additional fee.
          ...
          <div className="flex flex-wrap gap-2">
            {faqs.map((faq) => (
              <a
                key={faq.id}
                href={`#${faq.id}`}
                className="text-sm bg-white px-3 py-1.5 rounded-full border border-gray-200 text-gray-600 hover:border-[#e94560] hover:text-[#e94560] transition-colors"
              >
              ...
              >
                <div className="flex gap-4">
                  <div className="flex-shrink-0">
                    <span className="flex items-center justify-center w-10 h-10 rounded-full bg-[#16213e] text-white font-bold">
                      {index + 1}
                    </span>
                  </div>
                    ...
                    <div className="prose prose-lg max-w-none text-gray-600">
                      {faq.answer.split('\n\n').map((paragraph, pIndex) => {
                        if (paragraph.startsWith('**') && paragraph.endsWith('**')) {
                          return (
                            <h3 key={pIndex} className="text-lg font-semibold text-[#16213e] mt-6 mb-2">
                              {paragraph.replace(/\*\*/g, '')}
                            </h3>
                          );
                        }
