...
export const metadata: Metadata = {
  ...
  description: 'Common questions about notary public services in Stamford Hill, London. Learn about costs, the notarisation process, and more.',
};
              ...
              <article key={faq.id} id={faq.id} className="scroll-mt-24">
                <div className="flex gap-4">
                  <div className="flex-shrink-0">
                    <span className="flex items-center justify-center w-10 h-10 rounded-full bg-[#2c3e50] text-white font-bold">
                      {index + 1}
                    </span>
                  </div>
                    ...
                    <div className="prose prose-lg max-w-none text-gray-600">
                      {faq.answer.split('\n\n').map((paragraph, pIndex) => {
                        if (paragraph.startsWith('**') && paragraph.endsWith('**')) {
                          return (
                            <h3 key={pIndex} className="text-lg font-semibold text-[#2c3e50] mt-6 mb-2">
                              {paragraph.replace(/\*\*/g, '')}
                            </h3>
                          );
                        }
